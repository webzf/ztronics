...
```{=html}
<form id="selector-form" novalidate onsubmit="return window.embeddedNerdTouchscreenSubmit ? window.embeddedNerdTouchscreenSubmit(event) : false;">
```
```{=html}
...
/*
Expose the submit handler globally as a defensive fallback.
The inline form handler prevents a normal page refresh even if
the site's JavaScript initialization is delayed.
*/
