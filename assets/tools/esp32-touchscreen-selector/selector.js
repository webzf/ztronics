...
function applyPreset(id){var presets={"s3-psram":{family:"ESP32-S3",psram_min:8},"touch-spi":{display_present:true,touch:true,touch_interface:"SPI"},"native-usb":{native_usb:true},"large-display":{display_present:true,size_min:4}};var m=presets[id];if(!m)return;pendingPreset=id;reset(false);Object.keys(m).forEach(function(k){var ui=typeof m[k]==="boolean"?(m[k]?"yes":"no"):String(m[k]);var el=$(k);if(el){el.value=ui;var r=$(k+"-required");if(r){r.checked=true;requiredTouched[k]=true;}}});setMode("requirements");conditional();syncFilterDependencies();syncUrl();if(products.length){pendingPreset=null;lastEvaluation=Engine.evaluate(products,m,{});show(lastEvaluation);}else $("live-count").textContent="Preset selected · waiting for catalog…";}
function setupPresets(){window.EmbeddedNerdApplyPreset=applyPreset;document.addEventListener("click",function(e){var b=e.target&&e.target.closest?e.target.closest(".preset-link,.preset-button"):null;if(!b)return;e.preventDefault();var id=b.dataset.preset;if(!id){var href=b.getAttribute("href")||"";var m=href.match(/[#&?]preset=([^&]+)/);if(m)id=decodeURIComponent(m[1]);}if(id){applyPreset(id);history.replaceState(null,"",location.pathname+"#preset="+encodeURIComponent(id));}});window.addEventListener("hashchange",function(){var m=location.hash.match(/^#preset=(s3-psram|touch-spi|native-usb|large-display)$/);if(m)applyPreset(m[1]);});}
