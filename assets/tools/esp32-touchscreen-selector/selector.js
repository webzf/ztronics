...
function applyPreset(id){var presets={"s3-psram":{family:"ESP32-S3",psram_min:8},"touch-spi":{display_present:true,touch:true,touch_interface:"SPI"},"native-usb":{native_usb:true},"large-display":{display_present:true,size_min:4}};var m=presets[id];if(!m)return;pendingPreset=id;reset(false);Object.keys(m).forEach(function(k){var ui=typeof m[k]==="boolean"?(m[k]?"yes":"no"):String(m[k]);var el=$(k);if(el){el.value=ui;var r=$(k+"-required");if(r){r.checked=true;requiredTouched[k]=true;}}});setMode("requirements");conditional();syncFilterDependencies();syncUrl();updateLiveCount();pendingPreset=null;}function setupPresets(){window.EmbeddedNerdApplyPreset=applyPreset;var s=$("preset-select");if(s)s.addEventListener("change",function(){if(this.value){applyPreset(this.value);window.requestAnimationFrame(function(){var r=$("results");if(r)r.scrollIntoView({behavior:"smooth",block:"start"});});}});}function disableFilter(ids,off){ids.forEach(function(id){var e=$(id),r=$(id+"-required");if(e)e.disabled=off;if(r)r.disabled=off;if(off){if(e)e.value="";if(r)r.checked=false;requiredTouched[id]=false;}});}
