...
function applyPreset(id){var presets={"s3-psram":{family:"ESP32-S3",psram_min:8},"touch-spi":{display_present:true,touch:true,touch_interface:"SPI"},"native-usb":{native_usb:true},"large-display":{display_present:true,size_min:4},"c6-amoled-touch-battery":{family:"ESP32-C6",display_present:true,display_technology:"AMOLED",resolution:"368x448",touch:true,touch_type:"capacitive",touch_interface:"I2C",display_interface:"QSPI",battery:true,battery_charging:true}};var m=presets[id];if(!m)return;pendingPreset=id;reset(false);Object.keys(m).forEach(function(k){var ui=typeof m[k]==="boolean"?(m[k]?"yes":"no"):String(m[k]);var el=$(k);if(el){el.value=ui;var r=$(k+"-required");if(r){r.checked=true;requiredTouched[k]=true;}}});var ps=$("preset-select");if(ps)ps.value=id;setMode("requirements");conditional();syncFilterDependencies();syncUrl();updateLiveCount();pendingPreset=null;}
...
fetch("/assets/tools/esp32-touchscreen-selector/data/products.json?v=20260921-4")
  .then(function(r){
