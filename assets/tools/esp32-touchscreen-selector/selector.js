...
function setupPresets(){var box=document.querySelector(".preset-grid");if(!box)return;box.addEventListener("click",function(e){var b=e.target.closest(".preset-button");if(!b)return;e.preventDefault();var m={"s3-psram":{family:"ESP32-S3",psram_min:"8"},"touch-spi":{display_present:"yes",touch:"yes",touch_interface:"SPI"},"native-usb":{native_usb:"yes"},"large-display":{display_present:"yes",size_min:"4"}}[b.dataset.preset];if(!m)return;reset(false);Object.keys(m).forEach(function(k){var el=$(k);if(el){el.value=m[k];var r=$(k+"-required");if(r){r.checked=true;requiredTouched[k]=true;}}});setMode("requirements");conditional();updateLiveCount();syncUrl();var d=$( "advanced-filters");if(d&&(b.dataset.preset==="touch-spi"||b.dataset.preset==="large-display"))d.open=true;if(products.length)run();else $("live-count").textContent="Preset selected · waiting for catalog…";});}
