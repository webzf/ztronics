...
function setupPresets(){window.EmbeddedNerdApplyPreset=applyPreset;var s=$("preset-select");if(s)s.addEventListener("change",function(){if(this.value){applyPreset(this.value);window.requestAnimationFrame(function(){var r=$("results");if(r)r.scrollIntoView({behavior:"smooth",block:"start"});});}});}function disableFilter(ids,off){ids.forEach(function(id){var e=$(id),r=$(id+"-required");if(e)e.disabled=off;if(r)r.disabled=off;if(off){if(e)e.value="";if(r)r.checked=false;requiredTouched[id]=false;}});}
